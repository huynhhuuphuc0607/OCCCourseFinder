...
class DBHelper extends SQLiteOpenHelper {
    ...
    public void addOffering(Offering mOffering)
    {
        SQLiteDatabase db = getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(FIELD_CRN, mOffering.getCRN());
        values.put(FIELD_SEMESTER_CODE, mOffering.getSemesterCode());
        values.put(FIELD_COURSE_ID, mOffering.getCourse().getId());
        values.put(FIELD_INSTRUCTOR_ID, mOffering.getInstructor().getId());
    ...
    public List<Offering> getAllOfferings()
    {
        ...
        Cursor mCursor = db.query(OFFERINGS_TABLE,new String[]{FIELD_CRN, FIELD_SEMESTER_CODE, FIELD_COURSE_ID, FIELD_INSTRUCTOR_ID},
        ...
        if(mCursor.moveToFirst())
        {
            do{
                ...
                Offering offering = new Offering(mCursor.getInt(0), mCursor.getInt(1),
                       getCourse(mCursor.getLong(2)), getInstructor(mCursor.getLong(3)));
    ...
    {
        SQLiteDatabase db = getReadableDatabase();
        Cursor mCursor = db.query(OFFERINGS_TABLE,
        ...
        Course course = getCourse(mCursor.getLong(2));
        Instructor instructor = getInstructor(mCursor.getLong(3));
        Offering offering = new Offering(mCursor.getInt(0), mCursor.getInt(1), course, instructor);
